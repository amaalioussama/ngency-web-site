import style from "@/styles/Home/Services/style.css";
export default function ServiceComponent() {
  return (
    <div className="">
      <div className="border-4 border-l-4 border-t-0 border-b-0 -mr-1  ">
      
        
      <div className="border-2 border-black bg-mediumpink rotate-3 w-full h-[60px] flex justify-between items-center px-[15rem]">
        <h1 className="bg-[BB549B] text-white rotate-1 font-outfit text-[30px]">
          NOS SERVICES
        </h1>
        <h1 className="bg-[BB549B] text-white rotate-1 font-outfit text-[30px]">
          NOS SERVICES
        </h1>
        <h1 className="bg-[BB549B] text-white rotate-1 font-outfit text-[30px]">
          NOS SERVICES
        </h1>
      </div>
      <div className="grid grid-cols-4 gap-y-9 my-20 mx-14">
        <div className="section group block border-4 py-12 px-6 relative">
         <div className=''>
         <div className="circle group-hover:bg-mediumpink group-hover:border-black ">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
                className='group-hover:fill-white'
              />
            </svg>
          </div>
         </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
                className='group-hover:stroke-white'
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
     
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
        <div className="section border-4 py-12 px-6 relative">
          <div className="circle">
            <svg
              width="37"
              height="36"
              viewBox="0 0 47 46"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="absolute top-[9px] left-[9px]"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M3.39366 21.6527H22.2122V16.2331C19.5534 16.1138 16.9341 15.6421 14.4755 14.8329C11.7816 13.9462 9.32573 12.6691 7.25033 11.0621C5.04644 14.068 3.65347 17.7056 3.39366 21.6527ZM8.98613 8.99771C10.7789 10.3689 12.925 11.4857 15.3179 12.2733C17.499 12.9911 19.833 13.4182 22.2122 13.5355V2.83409C17.0252 3.1755 12.3727 5.47383 8.98613 8.99771ZM24.9069 2.83409V13.5355C27.5693 13.4042 30.1685 12.8854 32.5561 12.0109C34.6513 11.2435 36.5344 10.2204 38.133 8.99771C34.7464 5.47382 30.0939 3.1755 24.9069 2.83409ZM39.8688 11.0622C38.0135 12.4987 35.8514 13.6737 33.4829 14.5412C30.7898 15.5276 27.8755 16.1 24.9069 16.2331V21.6527H43.7255C43.4657 17.7056 42.0727 14.068 39.8688 11.0622ZM43.7255 24.3473H24.9069V29.7669C27.9219 29.9021 30.8813 30.4904 33.6099 31.5057C35.9298 32.3689 38.0476 33.5277 39.8688 34.9379C42.0727 31.932 43.4657 28.2945 43.7255 24.3473ZM38.133 37.0023C36.5646 35.8028 34.7214 34.7944 32.6702 34.0312C30.2511 33.1311 27.6114 32.5978 24.9069 32.4645V43.1659C30.0939 42.8245 34.7464 40.5262 38.133 37.0023ZM22.2122 43.1659V32.4645C19.644 32.5911 17.1312 33.0785 14.8085 33.9007C12.6154 34.6769 10.6474 35.7317 8.98613 37.0023C12.3728 40.5262 17.0252 42.8245 22.2122 43.1659ZM7.25033 34.9379C9.17697 33.4461 11.4342 32.2365 13.9093 31.3604C16.5281 30.4335 19.3461 29.8955 22.2122 29.7669V24.3473H3.39366C3.65347 28.2945 5.04644 31.932 7.25033 34.9379ZM0.654785 23C0.654785 10.35 10.9096 0.0952148 23.5596 0.0952148C36.2095 0.0952148 46.4644 10.35 46.4644 23C46.4644 35.65 36.2095 45.9048 23.5596 45.9048C10.9096 45.9048 0.654785 35.65 0.654785 23Z"
                fill="black"
              />
            </svg>
          </div>
          <h1 className="font-outfit font-black text-[15px] uppercase pb-4">
            Développement Web
          </h1>
          <p className="font-outfitRegular pb-6">
            Nous créons des sites web sur mesure en utilisant différents
            langages de programmation pour répondre à vos besoins spécifiques.
          </p>
          <div className="arrow flex items-center gap-2">
            <p className="font-outfit font-semibold uppercase">découvrir</p>
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M0 1H13M13 1V14M13 1L1 13"
                stroke="black"
                stroke-width="2"
              />
            </svg>
          </div>
        </div>
      </div>
      </div>
    </div>
  );
}
